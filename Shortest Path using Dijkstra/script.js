...
const indicatePath = async (parentArr, src, minCost) => {
  ...
  for (i = 0; i < cnt; i++) {
    ...
    let path = await printPath(parentArr, i, p);

    if (minCost[i] < minPathLength && i !== src) {
      minPathLength = minCost[i];
      minPathIndex = i;
      shortestPath = path;
    }
  }
  ...
  if (minPathIndex !== -1) {
    let shortestPathText = document.createElement("p");
    shortestPathText.innerText = `Shortest Path is Node ${minPathIndex} with cost ${minPathLength}: ${shortestPath.join(" -> ")}`;
    document.getElementsByClassName("path")[0].append(shortestPathText);
  }
};
...
const printPath = async (parent, j, el_p) => {
  ...
  return el_p.innerText.split(" ").map(Number).filter(n => !isNaN(n));
};
...
const highlightShortestPath = (path) => {
  for (let i = 0; i < path.length - 1; i++) {
    let n1 = path[i];
    let n2 = path[i + 1];
    let lineId = n1 < n2 ? `line-${n1}-${n2}` : `line-${n2}-${n1}`;
